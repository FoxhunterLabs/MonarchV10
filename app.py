...
Monarch V9.5 — Agnostic Autonomy Safety Kernel (Graph-Driven, FSM Policy, Replay-
Deterministic)
...
def __init__(self, mid, state, context, bus, audit, policy_fsm: PolicyFSM, config:
RiskConfig):
super().__init__(mid, state, context, bus, audit)
...
state={self.state.policy_version}"
...
# No tick/before_tick/after_tick logic needed for this module for now.
...
# Minimal consistency checks; extended vs V9
...
# mark expired
self.context.latest_intent = ActuationIntent(
id=li.id,
...
note=li.note + " (expired)",
)
self.audit.add(
"intent_expired",
f"Intent {li.id} expired",
{"intent_id": li.id, "tick": self.state.last_tick},
)
self.state.system_status = "DEGRADED"
self.state.system_status_reason = "Expired intent detected"
...
float, json_out: bool):
kernel = MonarchKernelV9_5(config=default_config(), seed=seed, mode=mode,
tick_budget_ms=tick_budget_ms)
print("Monarch V9.5 initialized.")
...
print(f" intent: level={li['level']} action={li['action']} op={li['operator_id']}{exp}")
if hb["muted_modules"]:
...
p = argparse.ArgumentParser("Monarch V9.5 — Autonomy Safety Kernel Demo")
